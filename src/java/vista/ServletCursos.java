 ...
 * @author busqu
 */
@WebServlet(name = "ServletCursos", urlPatterns = {"/ServletCursos.do"})
public class ServletCursos extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
         ...
         BeanCurso curso;
         curso = new BeanCurso(Id_curso, Nombredecurso, Id_profesor, Id_grado);
         boolean sw=Cursos.agregarCurso(curso);
         
         PrintWriter out=response.getWriter();
         out.println(Id_curso+Nombredecurso+ Id_profesor+ Id_grado);
         
         if(sw){
                            response.sendRedirect("http://registrodealumnos.com:8084/Registro/Guardado2.jsp");
